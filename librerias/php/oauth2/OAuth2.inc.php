 ...
 */
define("OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME", 86400);
...
abstract class OAuth2 {
   ...
   */
  abstract protected function setAccessToken($oauth_token, $client_id, $expires, $scope = NULL, $user_id);
  ...
  public function grantAccessToken() {
    ...
    // Grant Type must be specified.
    if (!$input["user_id"])
      $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_REQUEST, 'No existe el usuario');
    ...
    switch ($input["grant_type"]) {
      case OAUTH2_GRANT_TYPE_AUTH_CODE:
        ...
        if ($stored === NULL || (strcasecmp(substr($input["redirect_uri"], 0, strlen($stored["urlRedireccionar"])), $stored["urlRedireccionar"]) !== 0) || $client[0] != $stored["consumidorId"])
          $this->errorJsonResponse(OAUTH2_HTTP_BAD_REQUEST, OAUTH2_ERROR_INVALID_GRANT);

        if ($stored["expiracion"] < time())
    ...
    // Check scope, if provided
    if ($input["scope"] && (!is_array($stored) || !isset($stored["alcance"]) || !$this->checkScope($input["scope"], $stored["alcance"])))
  ...
  private function doRedirectUriCallback($redirect_uri, $params) {
    ...
    header("Location: " . $this->buildUri($redirect_uri, $params) . "&user_id=". $_SESSION['ID']);
    exit;
  }
   ...
   * @ingroup oauth2_section_4
   */
  protected function createAccessToken($client_id, $scope = NULL, $user_id) {
    $token = array(
